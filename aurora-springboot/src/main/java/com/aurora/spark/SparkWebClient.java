...
import com.aurora.spark.bean.RoleContent;
...
public class SparkWebClient {
    ...
    public WebSocket sendMsg(String uid, List<RoleContent> questions, WebSocketListener listener) {
        ...
        String authUrl = null;
        try {
            authUrl = getAuthUrl(sparkConfig.getHostUrl(), sparkConfig.getApiKey(), sparkConfig.getApiSecret());
        } catch (Exception e) {
            log.error("鉴权失败：{}", e);
            return null;
        }
